import type { Agent, Vec2 } from "@crowd-sim/shared";
...
function clampSpeed(v: Vec2, max: number): Vec2 {
  const speed = length(v);
  if (speed <= max || speed < 1e-8) return v;
 ...
 */
export function updateAgentPosition(agent: Agent, neighbors: Agent[]) {
  const toGoal = {
  ...
  for (const other of neighbors) {
    ...
    if (dist < 1e-6 || dist > INTERACTION_RADIUS) continue;
