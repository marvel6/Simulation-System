...
console.log(
  `[orchestrator] Started, interval=${LOOP_MS}ms, rebalance(env default)=${ENV_REBALANCE}`
);

async function isRebalanceEnabled(): Promise<boolean> {
  const flag = await redis.get(redisKeys.rebalanceEnabled());
  if (flag == null) return ENV_REBALANCE;
...
setInterval(() => {
  (async () => {
    ...
    console.log(
      `[orchestrator] H(P)=${h.toFixed(3)} rebalance=${rebalanceOn} | ${summary}`
    );
